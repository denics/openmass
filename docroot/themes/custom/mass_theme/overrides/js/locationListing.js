(function ($, Drupal) {
  ...
  function transformLocationListingPromos(promos) {
    ...
    let compiledTemplate = Handlebars.getTemplate('locationListingRow');
    let listingMarkup = [];
    let imagePromos = (promos.forEach === 'undefined') ? objToArray(promos) : promos;
    imagePromos.forEach(function (data, index) {
  ...
  function objToArray(myObj){
    let myArray = $.map(myObj, function(value, index) {
      return [value];
    });
    return myArray;
  }
