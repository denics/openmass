  ...
  public static function preparePressTeaser($entity, array $options = []) {
    ...
    if ($fields['org']) {
      if (!empty($entity->$fields['org']->entity)) {
        $org = $entity->$fields['org']->entity->field_state_org_ref_org->entity->getTitle();
      }
    }

    // On internal item, if empty, grab entity image.
    if ($fields['image'] && $options['displayImages'] == TRUE) {
      ...
      $description = [
        'rteElements' => [
          [
            ...
            'data' => [
              'paragraph' => [
                'text' => Helper::fieldValue($entity, $fields['description']),
              ],
            ],
          ],
        ],
