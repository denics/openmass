...
class MediaEntityDocumentNormalizer extends ContentEntityNormalizer {
  ...
  public function supportsNormalization($data, $format = NULL) {
    ...
    if ($data instanceof MediaInterface && $data->getType()->getPluginId() == 'document') {
      return TRUE;
    }
  ...
  public function normalize($object, $format = NULL, array $context = []) {
    ...
    if (isset($context['included_fields'])) {
      $attributes = array_intersect_key($attributes, array_flip($context['included_fields']));
    }
    ...
    if (!empty($attributes['license'])) {
      ...
      $specified_license = empty($attributes['field_other_license_url']) ? '' : $attributes['field_other_license_url']['uri'];
      $attributes['license'] = $this->getOpenLicense($license_text, $specified_license);
    }
    unset($attributes['field_other_license_url']);
    // If we ended up with an empty string license, then remove it from json.
    // This is to prevent the case when "Other" is selected but there is no
    // value in field_other_license_url or "License Not Specified" is selected.
    if (empty($attributes['license'])) {
    ...
    if (!empty($attributes['field_language'])) {
      ...
      $attributes['language'][] = $term->getName();
    }
  ...
  private function getIsoFrequency($frequency) {
    ...
    // If there is no ISO 8601 equivalent value then return 'irregular' to have
    // a valid json output (Once is the only value not recognized by ISO).
    // @see https://project-open-data.cio.gov/v1.1/schema/#accrualPeriodicity
    // Accepted Values line.
    return $frequency == 'Once' ? 'irregular' : $frequency;
  }
   ...
   * @param string $specified_license
   *   The value of the $attributes['field_other_license_url'].
   *
   * @return string
   ...
   */
  private function getOpenLicense($license, $specified_license) {
    $licenses = [
      ...
      'UK Open Government License (OGL)' => 'https://www.nationalarchives.gov.uk/doc/open-government-licence/version/2/',
    ];

    // Check fot "Other" value.
    if ($license == 'Other') {
      return $specified_license;
    }
